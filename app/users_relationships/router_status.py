...
from app.exceptions import UserNotFound, CannotSendRequestToYourself
...
@router.get("/friends")
async def get_outgoing_requests(current_user: User = Depends(get_current_user)):
    friends_names = await UserRelDAO.get_requests(user_from_id=current_user.id, relationship_status=RelationshipTypes.FRIENDS)
    return friends_names

@router.get("/status")
async def get_current_status_with_user(user_name: str, current_user: User = Depends(get_current_user)):
    ...
    relationship_data = await UserRelDAO.find_one_or_none(user_from_id=current_user.id, user_to_id=user_to.id)
